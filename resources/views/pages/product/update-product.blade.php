                ...
                <form action="{{ route('catalog.product.update.handler', $product->id) }}" method="post" enctype="multipart/form-data"
                    ...
                    <div class="card-body card-block">
                        ...
                        <div class="form-group">
                            ...
                            <input id="productName" type="text" name="name" value="{{ $product->name }}" class="form-control" required>
                            @error('name')
                                <div class="alert alert-danger mt-1" role="alert">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="row">
                            <div class="col">
                                <div class="form-group">
                                    ...
                                    <input id="productPrice" type="number" min="0" name="price" value="{{ $product->price }}" class="form-control" required>
                                    @error('price')
                                        <div class="alert alert-danger mt-1" role="alert">{{ $message }}</div>
                                    @enderror
                                </div>
                            ...
                            <div class="col">
                                <div class="form-group">
                                    <label for="productDiscountPercent" class="form-control-label">Discount percent (%)</label>
                                    <input id="productDiscountPercent" type="number" min="0" max="100" name="discountPercent"
                                        value="{{ $product->discount_percent }}" class="form-control" required>
                                    @error('price')
                                        <div class="alert alert-danger mt-1" role="alert">{{ $message }}</div>
                                    @enderror
                                </div>
